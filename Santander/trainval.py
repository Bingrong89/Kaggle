...
class Model1(nn.Module):
    ...
    def forward(self,x):
        ...
        output = self.linear3(F.relu(output))
    ...
    for epoch in range(num_epochs):
        ...
        for batch_count, data in enumerate(tqdm(batchtrain,desc="Epoch %s Training Phase"%epoch)):
            ...
            if batch_count in save_intervals:
                ...
                test_statement = "\n(PHASE::TEST) Epoch %i %i%% :::: Epoch Acc: %.2d%% ::Time Taken: %.2fs"\
...
trainbatch = DataLoader(traindataset,batch_size=32,sampler=sampler,drop_last=True)
testbatch = DataLoader(testdataset,batch_size=32,sampler=sampler2,drop_last=True)
...
train_model(model,trainbatch,testbatch,criterion,optimizer,10)
